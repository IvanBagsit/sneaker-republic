			...
			<Grid item className={styles.featuredBackground}>
				...
				>
					<Grid item className={styles.featuredTitle}>
						<Typography variant={`${device === "tablet" ? "h4" : "h5"}`}>
						...
						{featuredShoes?.map((items) => {
							return (
								<Card key={items.title} sx={cardDimension()}>
									...
									>
										<CardContent sx={cardContentDimension()}>
											<Typography
												variant={`${device === "tablet" ? "h5" : "h6"}`}
												component="div"
											>
												{items.title}
											</Typography>
				...
				<div className={`${styles.background} ${styles.list}`}>
					<div className={styles.content}>
						<div className={styles.contentItem}>
							<img src={ownerImage} alt="owner" className={styles.owner} />
						</div>
						<div className={`${styles.contentItem} ${styles.contentDetails}`}>
							<div>
								<h3>Welcome to Sneakers Republic!</h3>
							</div>
							<div>
								Hi! I'm Ivan and this is my partner Tin. We are the owner and
								creator of this e-commerce website. This project is being built
								to scale and help our small business in the shoes industry. As
								of now, this project doesn't have built-in payment feature yet
								but rest assured we are doing our best to improve our website.
							</div>
							<div
								style={{
									marginTop: "3%",
							...
							>
								<span style={{ marginRight: "2%" }}>
									For inquiries, please contact us on these platforms:
								</span>
								{contactDetails.map((items, index) => {
									return (
										<Tooltip arrow title={items.name} key={items.name}>
